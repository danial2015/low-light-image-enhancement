...
# --- CLAHE Enhancement ---
def apply_clahe(img, clip_limit=3.0, grid_size=(8,8)):
...
def color_restoration(img, alpha=125, beta=46):
    """Color restoration for MSRCR"""
    img_sum = np.sum(img, axis=2, keepdims=True)
    return beta * (np.log1p(alpha * img) - np.log1p(img_sum))

def MSRCR(img, sigma_list=SIGMA_LIST, G=5, b=25, alpha=125, beta=46):
...
def create_image_comparison_tab(results, light_mode=True):
    """Create tab with all enhanced images for comparison"""
    bg_color, text_color = set_theme(light_mode)
    ...
    # Create a grid of images - 2 columns
    cols_per_row = 2
    methods = list(results.keys())
    num_images = len(methods)
    
    # Display original first
    st.subheader("Original Image")
    st.image(results["Original"]["image"], use_container_width=True)
    
    # Display all enhanced methods
    st.subheader("Enhanced Results")
    
    # Group methods by type for better organization
    retinex_methods = [m for m in methods if m.startswith("Retinex")]
    other_methods = [m for m in methods if m not in ["Original"] and m not in retinex_methods]
    
    # Display other methods first
    ...
    # Display Retinex methods in an expander
    with st.expander("Retinex Variants", expanded=True):
        for method in retinex_methods:
            data = results[method]
            st.markdown(f"**{method}**")
            st.image(data["image"], 
                    caption=f"NIQE: {data['metrics']['NIQE']:.2f}, BRISQUE: {data['metrics']['BRISQUE']:.2f}, Time: {data['time']:.2f}s",
        ...
        try:
            ...
            with tabs[0]:
                ...
                with st.spinner("Processing with CLAHE..."):
                    ...
                    display_results(img_array, enhanced, original_metrics, metrics, exec_time, light_mode)
            ...
            with tabs[1]:
                ...
                with st.spinner(f"Processing with {method}..."):
                    start = time.time()
                    if method == "SSR":
                        enhanced = SSR(img_array, SIGMA_LIST[0])
                        # Convert to 8-bit image
                        enhanced = (enhanced - enhanced.min()) / (enhanced.max() - enhanced.min()) * 255
                        enhanced = enhanced.astype(np.uint8)
                    elif method == "MSR":
                        enhanced = MSR(img_array, SIGMA_LIST)
                        # Convert to 8-bit image
                        enhanced = (enhanced - enhanced.min()) / (enhanced.max() - enhanced.min()) * 255
                        enhanced = enhanced.astype(np.uint8)
                    elif method == "MSRCR":
                        enhanced = MSRCR(img_array)
                    else:  # MSRCP
                        enhanced = MSRCP(img_array)
                        
                    metrics = calculate_metrics(enhanced)
                    exec_time = time.time() - start
                    results[f"Retinex ({method})"] = {
                        "image": enhanced,
                        "metrics": metrics,
                        "time": exec_time
                    }
            ...
            with tabs[4]:
                create_image_comparison_tab(results, light_mode)
            
            # Metrics Comparison Tab
            with tabs[5]:
                if len(results) > 1:
                    display_metrics_comparison(results, light_mode)
                else:
                    st.warning("Process at least one enhancement method to see comparisons")
